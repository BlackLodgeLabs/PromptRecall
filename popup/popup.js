    ...
    chrome.storage.sync.get(['prompts'], (result) => {
      ...
      if (currentFilter) {
        prompts = prompts.filter(prompt =>
          ...
          prompt.site.toLowerCase().includes(currentFilter)
        );
      }
      ...
      for (const dateStr in groupedPrompts) {
        ...
        groupedPrompts[dateStr].forEach((prompt) => {
          ...
          actionsCell.className = 'prompt-actions'; // Add a class for styling
          ...
          row.appendChild(actionsCell); // Append the new actions cell
          promptList.appendChild(row);
        });
      }
